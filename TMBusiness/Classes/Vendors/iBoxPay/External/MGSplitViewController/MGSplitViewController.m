...
#define MG_DEFAULT_SPLIT_POSITION		290 //320.0	// default width of master view in UISplitViewController.
...
@implementation UIViewController (MGSplitViewControllerAddition)

- (MGSplitViewController *)mgSplitViewController {
    UIResponder* resp = nil;
    if ([self isKindOfClass:[UINavigationController class]]) {
        resp = self.view.superview.nextResponder;    
    }
    else if ([self isKindOfClass:[UIViewController class]]) {
        resp = self.navigationController.view.superview.nextResponder;
    }
    
    if (resp && [resp isMemberOfClass:[MGSplitViewController class]]) {
        return (MGSplitViewController *)resp;
    }
