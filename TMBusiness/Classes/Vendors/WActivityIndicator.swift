...
class WActivityIndicator: UIView {
    ...
    private var indicatorWidth:Float = 0.0
    private var indicatorHeight:Float = 0.0
    ...
    override func layoutSubviews () {
        ...
        self.indicatorWidth = Float(indicatorFrame!.size.width) + (2.0 * margin)
        self.indicatorHeight = Float(indicatorFrame!.size.height) + (2.0 * margin)
        
        var contentViewFrameX = floor((Float(frame.size.width) - self.indicatorWidth) / 2 )
        var contentViewFrameY = floor((Float(frame.size.height) - self.indicatorHeight) / 2 )
        
        contentViewFrame = CGRect(x: CGFloat(contentViewFrameX), y: CGFloat(contentViewFrameY), width: CGFloat(self.indicatorWidth), height: CGFloat(self.indicatorHeight))
        ...
        if count(labelText) != 0 {
            ...
            // 设置装空间view 位置
            if stringWidth > self.indicatorWidth {
                self.indicatorWidth = stringWidth
                contentViewFrameX = floor((Float(frame.size.width) - self.indicatorWidth) / 2 )
                contentViewFrame!.origin.x = CGFloat(contentViewFrameX)
                contentViewFrame!.size.width = CGFloat(self.indicatorWidth)
            }
